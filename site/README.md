...
Deploy: Cloudflare Pages, project root `site`, build command empty, output directory `/`. Custom domains `wordsnap.ai` and `www.wordsnap.ai` (DNS is already on Cloudflare). `_headers` sets the security headers.
