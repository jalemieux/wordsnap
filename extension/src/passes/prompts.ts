...
If the text is not a draft yet but notes for one (fragments, a line saying who it is for or what it is about, a list of points, no complete argument), return verdict "outline": the paragraphs the message needs, in order, as "slots". Each slot has "role" (one to three plain words: "Ask", "Evidence", "What happened"), "job" (one plain line on what that paragraph does for the reader), "from" (the writer's own fragments that belong in it, quoted exactly from the notes; empty when the notes give it nothing) and, when the notes leave out something the reader will need, "gap" (one line naming it: a number, a name, what happened, by when). Do not write the paragraphs and do not reword the fragments. Every fragment goes in at most one slot; a line that only says who the message is for or what it is about goes nowhere. "paragraphs" stays empty. "note" says in one or two sentences what order the outline follows and which slots have nothing yet.
