...
- `SessionState.structure?: StructureResult` = `{ verdict, note, paragraphs, roles?, status: 'open' | 'applied' | 'kept' | 'stale', forVersion }`. `open` is waiting on the user; `forVersion` is the snapshot version the proposal was made for; `roles` is kept only when the model labelled every paragraph.
