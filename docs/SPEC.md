...
- From notes (September 2026). When the text is notes rather than a draft (fragments, a line saying who it is for, no complete argument) pass S returns verdict `outline` (`PassS.slots`, additive: role, job, the writer's own fragments quoted from the notes, an optional gap naming what the notes leave out). The compare pane shows it the same way: the notes on the left with a tag per fragment saying which paragraph it feeds, the slots on the right. **Apply outline** writes only the fragments into the editor, one paragraph per slot in that order, through `minimalParagraphEdit`; the roles, job lines and gaps are never written. The result stays open as a guide (status `guiding`): edits do not stale it, nothing runs, and the pane marks each slot empty, seeded or written from the draft alone (`outlineFill`, no model call). **Done** (`structure/action: done`) closes the guide and runs everything on what was written, S included; Re-analyze does the same. Keep mine checks the notes as they are. The validator drops a fragment that is not in the notes and demotes an outline that places none, or has fewer than two slots, to `keeps`. Detection is the pass's call; there is no chip. Over a narrow draft the guide moves into the panel as a checklist so the editor stays free to type into.
...
**Entry point.** WordSnap is quiet by default. A 32 px launcher badge sits inside the compose frame, top right under the host's title bar. Nothing is sent anywhere until the user clicks it; the click arms the session, the panel opens, and analysis starts (minimum 8 words). While collapsed, the badge shows a spinner during a run and afterwards a count of open issues. A setting, off by default, restores automatic analysis once a draft passes 40 words.
...
- **ChallengesPanel.** Docked to the right of `anchorRect`, falls back to a floating panel when there is no room. Header with status pill (Checked just now / Re-analyzing), the four check chips, summary counts, one line on the structure pass (`keeps` and why, or a pointer to the compare pane while a `reorder` is open; A, B and C wait on that choice), "Your argument as WordSnap reads it," then the challenge list with strongest rebuttal first. Hovering a challenge highlights its anchor sentences. A challenge whose anchors changed is re-evaluated and, if the new text answers it, shown as addressed.
